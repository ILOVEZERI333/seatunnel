...
public class AzureCosmosDBConfig implements Serializable {
    ...
    public AzureCosmosDBConfig(ReadonlyConfig config) {
        ...
        this.schema = ReadonlyConfig.fromMap(config.get(ConnectorCommonOptions.SCHEMA)).toConfig();

        if (getResolvedEndpoint() == null) {
            throw new IllegalArgumentException(
                    "AzureCosmosDB source requires uri, endpoint, or connection string to resolve the endpoint");
        }
        if (getResolvedKey() == null) {
            throw new IllegalArgumentException(
                    "AzureCosmosDB source requires key, primary_key, secondary_key, or a connection string to resolve the key");
        }
    }
